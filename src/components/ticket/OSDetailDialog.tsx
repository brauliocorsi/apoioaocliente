...
import { Loader2, Printer, Package, Truck, CreditCard, FileText, User, Hash, Wrench } from "lucide-react";
...
export default function OSDetailDialog({ open, onOpenChange, osId, osCodigo }: OSDetailDialogProps) {
  ...
  const fetchOS = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("gestaoclick-proxy", {
        body: { action: "get_os", id: osId },
      });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
      setOs(data?.data || data);
    } catch (e: any) {
      console.error("Fetch OS error:", e);
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`<!DOCTYPE html><html><head><title>OS #${osCodigo}</title>
      <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .obs-box { background: #f9f9f9; border: 1px solid #e5e5e5; border-radius: 4px; padding: 8px; font-size: 10px; white-space: pre-wrap; }
  ...
  const produtos = (os?.produtos || []).map((p: any) => p.produto || p);
  const servicos = (os?.servicos || []).map((s: any) => s.servico || s);
  const pagamentos = (os?.pagamentos || []).map((p: any) => p.pagamento || p);
  const atributos = (os?.atributos || []).map((a: any) => a.atributo || a);
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-sm text-muted-foreground">A carregar dados da OS...</p>
          </div>
        )}
        ...
        {os && (
          <div ref={contentRef} className="space-y-5">
            ...
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-xl font-bold text-primary">OS #{os.codigo || osCodigo}</h2>
                <p className="text-sm text-muted-foreground">
                  {fmtDate(os.data)} · {os.nome_loja || ""}
                </p>
              </div>
              <div className="text-right space-y-1">
                <Badge
                  className="text-xs"
                  style={{
                    backgroundColor: os.cor_situacao || undefined,
                    color: os.cor_situacao ? "#333" : undefined,
                  }}
                >
                  {os.nome_situacao || "–"}
                </Badge>
                <p className="text-lg font-bold">{fmt(os.valor_total)}</p>
              </div>
            </div>
            ...
            <div>
              <h3 className="text-xs font-semibold uppercase text-primary mb-3 flex items-center gap-1.5">
                <User className="h-3.5 w-3.5" /> Informações da OS
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1.5 text-sm">
                <InfoRow label="Cliente" value={os.nome_cliente} />
                <InfoRow label="Vendedor" value={os.nome_vendedor} />
                <InfoRow label="Centro de Custo" value={os.nome_centro_custo} />
                <InfoRow label="Canal de Venda" value={os.nome_canal_venda} />
                <InfoRow label="Prazo Entrega" value={fmtDate(os.prazo_entrega)} />
                <InfoRow label="Data Início" value={fmtDate(os.data_inicio)} />
                <InfoRow label="Data Fim" value={fmtDate(os.data_fim)} />
                <InfoRow label="Condição Pgto" value={os.condicao_pagamento === "parcelado" ? "Parcelado" : os.condicao_pagamento === "a_vista" ? "À vista" : os.condicao_pagamento} />
                <InfoRow label="Forma Pgto" value={os.nome_forma_pagamento} />
                <InfoRow label="Nº Parcelas" value={os.numero_parcelas} />
                {os.nome_transportadora && <InfoRow label="Transportadora" value={os.nome_transportadora} />}
                {os.equipamento && <InfoRow label="Equipamento" value={os.equipamento} />}
                {os.numero_serie && <InfoRow label="Nº Série" value={os.numero_serie} />}
                {os.defeito_relatado && <InfoRow label="Defeito Relatado" value={os.defeito_relatado} />}
                {os.laudo_tecnico && <InfoRow label="Laudo Técnico" value={os.laudo_tecnico} />}
                <InfoRow label="Criado em" value={os.cadastrado_em} />
                <InfoRow label="Modificado em" value={os.modificado_em} />
              </div>
            </div>
            ...
            <div className="flex justify-end">
              <div className="w-64 space-y-1 text-sm">
                {parseFloat(os.valor_produtos || "0") > 0 && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Produtos:</span>
                    <span>{fmt(os.valor_produtos)}</span>
                  </div>
                )}
                {parseFloat(os.valor_servicos || "0") > 0 && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Serviços:</span>
                    <span>{fmt(os.valor_servicos)}</span>
                  </div>
                )}
                {parseFloat(os.desconto_valor || "0") > 0 && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Desconto:</span>
                    <span>-{fmt(os.desconto_valor)}</span>
                  </div>
                )}
                {parseFloat(os.valor_frete || "0") > 0 && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Frete:</span>
                    <span>{fmt(os.valor_frete)}</span>
                  </div>
                )}
                <Separator />
                <div className="flex justify-between font-bold text-base text-primary pt-1">
                  <span>Total:</span>
                  <span>{fmt(os.valor_total)}</span>
                </div>
              </div>
            </div>
